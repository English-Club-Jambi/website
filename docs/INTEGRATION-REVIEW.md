...
| CMS capacity | Practice content exceeded the former 200-row assumption | Public/admin content functions share a 256-entry page/locale ceiling. Tests read all 212 current Practice keys, allow 256, reject entry 257, and detect an over-limit legacy page. |
...
- Assessment backend tests cover identity ownership, pre-submit key privacy, early-submit rejection, current-section expiry, private/wrong-version media rejection, launch-gate behavior, 256-entry CMS capacity, authoring/review, reorder/delete, and attempt deletion.
