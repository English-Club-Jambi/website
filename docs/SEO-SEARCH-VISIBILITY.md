...
- Every indexable public route emits one absolute HTTPS canonical URL on `englishclubjambi.my.id` unless an explicit non-local HTTPS origin is configured.
...
```dotenv
NEXT_PUBLIC_SITE_URL=https://englishclubjambi.my.id
...
5. Submit `https://englishclubjambi.my.id/sitemap.xml` to both tools. The sitemap is a discovery hint, not an indexing command.
